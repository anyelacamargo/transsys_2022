...
# $Id: __init__.py 371 2012-07-24 11:43:22Z jtk $
...
import string
#import StringIO
from io import StringIO # Modyfied
import math
import re
import random
...
required_clib_api_version = '350'
 ...
 # raise Exception('transsys module requires clib API version %s but actual API version is %s' % (required_clib_api_version, clib.clib_api_version))
...
"""
  ...
  def __init__(self, factor, transsys_label = None) :
    if not (isinstance(factor, Factor) or (type(factor) is bytes)) :
      raise Exception('ExpressionNodeIdentifier::__init__: bad type of factor')
    self.factor = factor
  ...
  def resolve(self, tp) :
    if isinstance(self.factor, Factor) :
      raise Exception('ExpressionNodeIdentifier::resolve: identifier is already resolved')
...
class ExpressionNodeBinary(ExpressionNode) :
  ...
  def __init__(self, op1, op2, operator_sym = None) :
    if not isinstance(op1, ExpressionNode) :
      raise Exception('ExpressionNodeBinary::__init__: bad type of op1')
    if not isinstance(op2, ExpressionNode) :
      raise Exception('ExpressionNodeBinary::__init__: bad type of op2')
    self.operand1 = op1
    self.operand2 = op2
...
class ExpressionNodeNot(ExpressionNode) :
  ...
  def __init__(self, operand) :
    if not isinstance(operand, ExpressionNode) :
      raise Exception('ExpressionNodeNot::__init__: bad type of operand')
...
class ExpressionNodeFunction(ExpressionNode) :
  ...
  def __init__(self, operand, funcname = 'undefined_function') :
    for op in operand :
      if not isinstance(op, ExpressionNode) :
        raise Exception('ExpressionNodeFunction::__init__: bad type of operand')
    self.operand = operand
  ...
  def evaluate(self, transsys_instance) :
    raise Exception('cannot evaluate undefined function')
...
class PromoterElementConstitutive(PromoterElement) :
  ...
  def __init__(self, expr) :
    if not isinstance(expr, ExpressionNode) :
      raise Exception('ExpressionNodeNot::__init__: bad type of expr')
...
complex identifiers."""
  ...
  def find_identified_factor_list(self) :
    factor_list = []
    for factor in [idnode.factor for idnode in self.getIdentifierNodes()] :
...
# expr1, expr2, ... should perhaps become a list at some stage...
  def __init__(self, expr1, expr2, factor_list, dot_attrs = None) :
    ...
    if not isinstance(expr1, ExpressionNode) :
      raise Exception('ExpressionNodeNot::__init__: bad type of expr1')
    if not isinstance(expr2, ExpressionNode) :
      raise Exception('ExpressionNodeNot::__init__: bad type of expr2')
...
any identifier nodes that may appear in the parameters to the link element."""
    ...
    for factor in self.factor_list :
      if not isinstance(factor, Factor) :
        raise Exception('PromoterElementLink::getIdentifierNodes: non-Factor in factor_list (unresolved transsys?)')
  ...
  def find_identified_factor_list(self) :
    factor_list = []
    for factor in [idnode.factor for idnode in self.expression1.getIdentifierNodes() + self.expression2.getIdentifierNodes()] :
      if factor not in factor_list :
        factor_list.append(factor)
    return factor_list
...
"""
  def __init__(self, name, decay_expr, diffusibility_expr, synthesis_expr, dot_attrs = None) :
    if decay_expr is not None and not isinstance(decay_expr, ExpressionNode) :
      raise Exception('Factor::__init__: bad decay_expr type')
    if diffusibility_expr is not None and not isinstance(diffusibility_expr, ExpressionNode) :
      raise Exception('Factor::__init__: bad diffusibility_expr type')
    if synthesis_expr is not None and not isinstance(synthesis_expr, ExpressionNode) :
      raise Exception('Factor::__init__: bad synthesis_expr type')
    self.name = name
...
"""
  #FIXME: I DON'T UNDERSTAND THE POINT OF THIS FIRST STATEMENT AS WHAT IS PASSED IS A STRING
  def __init__(self, name, product, promoter = None, dot_attrs = None) :
   # if not (isinstance(product, Factor) or (type(product) is bytes)) :
    #  raise Exception('Gene::__init__: bad type of product')
    self.name = name
    ...
    if promoter is None :
      ...
      for p in promoter :
        if not isinstance(p, PromoterElement) :
          raise Exception('Gene::__init__: bad element in promoter')
...
"""
    ...
    if len(self.promoter) != len(other.promoter) :
      return False
    for i in range(len(self.promoter)) :
    ...
    if factor_list is None :
      ...
      for f in factor_list :
        if not isinstance(f, Factor) :
          raise Exception('TranssysProgram::__init__: unsuitable element in factor_list')
    ...
    if gene_list is None :
      ...
      for g in gene_list :
        if not isinstance(g, Gene) :
          raise Exception('TranssysProgram::__init__: unsuitable element in gene_list')
...
"""
    ...
    if isinstance(f_name, Factor) :
      f_name = f_name.name
    #elif type(f_name) is not bytes : # AVCR
    #  raise Exception('TranssysProgram::find_factor: illegal argument type')
    i = self.find_factor_index(f_name)
    if i < 0 :
      raise Exception('TranssysProgram::find_factor: no factor "%s" in transsys %s' % (f_name, self.name))
...
"""
    if isinstance(g_name, Gene) :
      g_name = g_name.name
    if type(g_name) is not bytes :
      raise Exception('TranssysProgram::find_gene: illegal argument type')
    i = self.find_gene_index(g_name)
    if i < 0 :
      raise Exception('TranssysProgram::find_gene: no gene "%s" in transsys %s' % (g_name, self.name))
...
"""
    ...
    for factor in self.factor_list :
      if factor.name in fnames :
        raise Exception('factor name "%s" duplicated' % factor.name)
      fnames.append(factor.name)
    gnames = []
    for gene in self.gene_list :
      if gene.name in self.gene_list :
        raise Exception('gene name "%s" duplicated' % gene.name)
...
genes in this transsys program."""
  ...
  def regulated_genes(self, factor) :
    ...
    if type(factor) is bytes :
      i = self.find_factor_index(factor)
      if i == -1 :
        raise Exception('TranssysProgram.regulated_genes: transsys program "%s" does not contain factor "%s"' % (self.name, factor))
      factor = self.factor_list[i]
    #if not isinstance(factor, Factor) : # AVCR FIXME, this statetement is incorrect, factor is a string
...
class Symbol(object) :
  ...
  def transsys_name(self) :
    ...
    elif type(self.transsys) is bytes :
      return self.transsys
    else :
      raise Exception('Symbol::transsys_name: bad type %s of transsys member' % str(type(self.transsys)))
  ...
  def associate_transsys(self, tp) :
    ...
    if tp.name != self.transsys_name() :
      raise Exception('associating incompatible transsys: "%s" != "%s"' % (tp.name, self.transsys_name()))
    if self.graphics is not None :
      for g in self.graphics :
...
class LsysProgram(object) :
  ...
  def find_symbol(self, symbol_name) :
    if type(symbol_name) is not bytes :
      raise Exception('LsysProgram::find_symbol: illegal argument type')
    i = self.find_symbol_index(symbol_name)
    if i < 0 :
      raise Exception('LsysProgram::find_symbol: no symbol "%s" in lsys %s' % (symbol_name, self.name))
  ...
  def find_rule(self, rule_name) :
    if type(rule_name) is not bytes :
      raise Exception('LsysProgram::find_rule: illegal argument type')
    i = self.find_rule_index(rule_name)
    if i < 0 :
      raise Exception('LsysProgram::find_rule: no rule "%s" in lsys %s' % (rule_name, self.name))
    return self.rules[i]
...
class TranssysInstance(object) :
  ...
  def __init__(self, transsys_program, timestep = None) :
    if not isinstance(transsys_program, TranssysProgram) :
      raise Exception('TranssysInstance::__init__: unsuitable type of transsys_program (no TranssysProgram)')
  ...
  def __str__(self) :
    ...
    s = s + 'timestep: %s\n' % str(self.timestep)
    for f in range(len(self.factor_concentration)) :
...
"""
    ...
    if factor_index == -1 :
      raise Exception('factor "%s" not in transsys program "%s"' % (factor, self.transsys_program.name))
...
copy.deepcopy on it."""
  ...
  def set_uniform_random(self, rng, c_min, c_max) :
    ...
    c_range = c_max - c_min
    for i in range(len(self.transsys_program.factor_list)) :
...
"""
    if self.transsys_program is not other.transsys_program :
      raise Exception('transsys programs are not identical')
    s = 0.0
    for i in range(len(self.factor_concentration)) :
  ...
  #     subclass
  # FIXME: num_timesteps is a badly named parameter as it really specifies the number of
  #     instances to be created, not the number of timesteps (number of timesteps is effectively
  #     num_timesteps - 1
  def time_series(self, num_timesteps, sampling_period = 1, lsys_lines = None, lsys_symbol = None) :
    if lsys_lines is not None or lsys_symbol is not None :
      raise Exception('lsys_lines and lsys_symbol are no longer supported -- try using time_series_old')
...
"""
  ...
  def __str__(self) :
    ...
    for i in range(self.transsys_program.num_factors()) :
...
are required, the instances should be cloned.
    """
    raise Exception('transsys_instance_list not overridden by subclass')
...
"""
    if self.transsys_program is None :
      raise Exception('no transsys program, cannot write header')
    f.write('timestep')
...
"""
  ...
  def get_transsys_program(self) :
    ...
    raise Exception('get_transsys_program not overridden by subclass')
  ...
  def add(self, ti) :
    if self.transsys_program is None :
      ...
      if self.transsys_program is not ti.transsys_program :
        raise Exception('members of collection must be instances of the same program')
...
class SymbolInstance(object) :
  ...
  def __init__(self, symbol, transsys_instance = None, rule = None) :
    if not isinstance(symbol, Symbol) :
      raise Exception('symbol must be an instance of Symbol')
    ...
    if symbol.transsys is not None :
      if symbol.transsys is not transsys_instance.transsys_program :
        raise Exception('symbol and transsys_instance are not compatible')
    self.symbol = symbol
    ...
    for sym in symbol_list :
        if sym.transsys_instance is not None :
            ...
            if tp is not sym.transsys_instance.transsys_program :
                raise Exception('lsys has transsys instances of multiple transsys programs')
...
all methods will have to be overridden anyway."""
  ...
  def __init__(self, l, start = None) :
    if len(l) == 0 :
      raise Exception('CyclicSequence::__init__: cannot init with empty list')
  ...
  def from_string(self, s) :
    ...
    while s :
      ...
      if m is None :
        m = string_re.match(s)
        if m is None :
          raise Exception('CyclicSequence::from_string: malformed string "%s"' % s)
      l.append(m.group(1))
      s = s[len(m.group(0)):]
    self.i = len(l) - 1
    self.l = l
...
"""
    ...
    if self.topology == 'random_nk' :
      ...
      raise Exception('RandomTranssysParameters::write: unknown topology type "%s"' % self.topology)
    s = s + 'topology: end\n'
...
so it's best to require explicit specification of all parameters."""
    ...
    def parse_cyclicseq(f, label) :
      ...
      if m is None :
        raise Exception('RandomTranssysParameters::parse: failed to obtain CyclicSequence "%s"' % (label, line.strip()))
    ...
    line = f.readline()
    if str.split(line) != str.split(self.savefile_magic) : ## Modified
      raise Exception('RandomTranssysParameters::parse: bad magic "%s"' % str.split(line))

    line = f.readline()
    m = re.match('topology\\s*:\\s*([A-Za-z_]+)', line)
    if m is None :
      raise Exception('RandomTranssysParameters::parse: failed to obtain topology')
    # print 'topology type: "%s"' % m.group(1)
    self.topology = m.group(1)
    if self.topology == 'random_nk' :
      self.n = utilidad.parse_int(f, 'n')
      self.k = utilidad.parse_int(f, 'k')
    elif self.topology == 'random_uniform' :
      self.n = utilidad.parse_int(f, 'n')
      self.num_edges = utilidad.parse_int(f, 'num_edges')
    elif self.topology == 'random_powerlaw' :
      self.n = utilidad.parse_int(f, 'n')
      self.num_edges = utilidad.parse_int(f, 'num_edges')
      self.power_exp = utilidad.parse_float(f, 'power_exp')
      self.power_base = utilidad.parse_float(f, 'power_base')
    elif self.topology == 'linklist' :
      ...
      raise Exception('RandomTranssysParameters::parse: unknown topology type %s' % self.topology)
    line = f.readline()
    m = re.match('topology\\s*:\\s*end', line.strip())
    if m is None :
      raise Exception('RandomTranssysParameters::parse: topology improperly terminated by "%s"' % line.strip())
    ...
    rndseed = utilidad.parse_int(f, 'rndseed')
    self.set_seed(rndseed)
    line = f.readline()
    if line.strip() != '' :
      raise Exception('RandomTranssysParameters::parse: trailing garbage line "%s"' % line.strip())
    ...
    for n in self.linklist :
      for i in n :
        if (i < 0 and -i - 1 >= len(t)) or i >= len(t) :
          raise Exception('RandomTranssysParameters::check_linklist: illegal index %d' % i)
  ...
  def num_genes(self) :
    if self.topology == 'random_nk' or self.topology == 'random_uniform' or self.topology == 'random_powerlaw' :
      ...
      raise Exception('RandomTranssysParameters::num_genes: unknown topology type "%s"' % self.topology)
  ...
  def num_links(self) :
    if self.topology == 'random_nk' :
      ...
      raise Exception('RandomTranssysParameters::num_links: unknown topology type "%s"' % self.topology)
...
function."""
    ...
    def random_nk_linklist(n, k, rng) :
      if rng is None :
        raise Exception('RandomTranssysParameters::generate_transsys: cannot generate random NK linklist without RNG')
      linklist = []
      for i in range(n) :
        linklist.append([])
        in_list = list(range(n))
        for j in range(k) :
    ...
    def random_powerlaw_linklist(num_genes, num_edges, power_exp, power_base, rng) :
      ...
      rw = [x / rw_sum for x in rw[:-1]]
      # print 'rw (normalised to [0, 1[)', rw
      rw_borders = utilidad.interval_list(rw)
      # print 'rw_borders', rw_borders
      for i in range(num_edges) :
        # print 'edge #%d' % i
        g0 = utilidad.find_interval_index(rng.random(), rw_borders)
        g1 = utilidad.find_interval_index(rng.random(), rw_borders)
...
This function does not create any multilinks."""
      ...
      if len(all_links) < num_edges :
        raise Exception('number of edges %d too large: max %d for %d nodes' % (num_edges, len(all_links), num_genes))
      linklist = []
      for i in range(num_genes) :
        linklist.append([])
      for i in range(num_edges) :
    ...
    def next_expression(s) :
      p = TranssysProgramParser(io.StringIO(s.nextval()))
      return p.parse_expr([])
    ...
    for i in range(self.num_genes()) :
      ...
      synthesis_expr = next_expression(self.synthesis) ## Modified, added this line
      
      flist.append(Factor(factor_name(i), decay_expr, diffusibility_expr, synthesis_expr)) ## AVCR
      promoter = []
      ...
      glist.append(Gene(gene_name(i), flist[i], promoter))
...
class TranssysProgramScanner(object) :
  ...
  def isdelimiter(self, c) :
    if len(c) != 1 :
      raise Exception('attempt to classify multicharacter string as delimiter')
    if c.isspace() :
      return False
    if c in '_' :
      return False
  ...
  def get_token(self) :
    ...
    while self.buffer == '' :
      ...
      if self.buffer == '' :
        return None, None
      self.buffer = str.strip(self.buffer)
      if len(self.buffer) > 0 :
        if self.buffer[0] == '#' or self.buffer[0:2] == '//' :
          self.buffer = ''
    for kw in self.keywords :
      if self.buffer[:len(kw)] == kw :
        ...
        if re.match('%s($|[^A-Za-z0-9_])' % kw, self.buffer) is not None :
          self.buffer = str.strip(self.buffer[len(kw):])
          return kw, None
    m = self.identifier_re.match(self.buffer)
    if m :
      s = m.group()
      self.buffer = str.strip(self.buffer[len(s):])
      return ('identifier', s)
    ...
    if m :
      ...
      return ('realvalue', v)
    c = self.buffer[0]
    self.buffer = str.strip(self.buffer[1:])
    return c, None
    raise Exception('line %d: scanner stalled at "%s"' % (self.lineno, self.buffer))
...
class TranssysProgramParser(object) :
  ...
  def expect_token(self, expected_token) :
    ...
    if t != expected_token :
      raise Exception('line %d: expected token "%s" but got "%s"' % (self.scanner.lineno, expected_token, t))
    return v
  ...
  def parse_identifier_expr(self, transsys_label_list) :
    ...
    if self.consume_if_found('.') :
      if id not in transsys_label_list :
        raise Exception('unknown transsys label "%s"' % id)
      transsys_id = id
      id = self.expect_token('identifier')
    else :
      transsys_id = None
  ...
  def parse_argument_list(self, transsys_label_list) :
    ...
    while sep == ',' :
      ...
      if sep != ',' and sep != ')' :
        raise Exception('line %d: expected token "," or ")" in arglist but got "%s"' % (self.scanner.lineno, sep))
    return arglist
  ...
  def parse_gauss_expr(self, transsys_label_list) :
    ...
    if len(arglist) != 2 :
      raise Exception('line %d: gauss() takes 2 parameters, but got %d' % (self.scanner.lineno, len(arglist)))
  ...
  def parse_power_expr(self, transsys_label_list) :
    ...
    arglist = self.parse_argument_list(transsys_label_list)
    if len(arglist) != 2 :
      raise Exception('line %d: pow() takes 2 parameters, but got %d' % (self.scanner.lineno, len(arglist)))
  ...
  def parse_logarithm_expr(self, transsys_label_list) :
    ...
    arglist = self.parse_argument_list(transsys_label_list)
    if len(arglist) != 2 :
      raise Exception('line %d: log() takes 2 parameters, but got %d' % (self.scanner.lineno, len(arglist)))
  ...
  def parse_atan_expr(self, transsys_label_list) :
    ...
    arglist = self.parse_argument_list(transsys_label_list)
    if len(arglist) != 1 :
      raise Exception('line %d: atan() takes 1 parameter, but got %d' % (self.scanner.lineno, len(arglist)))
  ...
  def parse_value_expr(self, transsys_label_list) :
    ...
    if l == 'realvalue' :
      ...
      raise Exception('line %d: got token "%s", but value must be either <realvalue>, <identifier>, "random", "gauss", "pow", "log", or "atan"' % (self.scanner.lineno, l))
  ...
  def parse_promoter_statement(self) :
    ...
    if self.consume_if_found('activate') :
      ...
      if len(arglist) != 2 :
        raise Exception('line %d: activate() takes exactly 2 parameters, got %d' % (self.scanner.lineno, len(arglist)))
      self.expect_token(';')
      return PromoterElementActivate(arglist[0], arglist[1], fc)
    elif self.consume_if_found('repress') :
      arglist = self.parse_argument_list([])
      if len(arglist) != 2 :
        raise Exception('line %d: repress() takes exactly 2 parameters, got %d' % (self.scanner.lineno, len(arglist)))
      self.expect_token(';')
      return PromoterElementRepress(arglist[0], arglist[1], fc)
    raise Exception('line %d: bad token "%s" in promoter statement' % (self.scanner.lineno, self.scanner.lookahead()))
    ...
    while 1 :
      ...
      if l == 'decay' :
        if decay_expr is not None :
          raise Exception('line %d: duplicate decay expression' % self.scanner.lineno)
        self.expect_token('decay')
        self.expect_token(':')
        decay_expr = self.parse_expr([])
      elif l == 'diffusibility' :
        if diffusibility_expr is not None :
          raise Exception('line %d: duplicate diffusibility expression' % self.scanner.lineno)
        self.expect_token('diffusibility')
        self.expect_token(':')
        diffusibility_expr = self.parse_expr([])
      elif l == 'synthesis' :
        if synthesis_expr is not None :
          raise Exception('line %d: duplicate synthesis expression' % self.scanner.lineno)
  ...
  def parse_lhs_symbol(self, symbol_dict) :
    symbol_name = self.expect_token('identifier')
    if symbol_name not in list(symbol_dict.keys()) :
      raise Exception('unknown symbol "%s"' % symbol_name)
  ...
  def parse_rule(self, symbol_dict) :
    ...
    transsys_label_list = [x.transsys_label for x in lhs]
    if self.consume_if_found(':') :
  ...
  def parse_move(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 1 :
      raise Exception('move takes 1 argument, got %d' % len(arglist))
  ...
  def parse_push(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 0 :
      raise Exception('push does not take arguments, got %d' % len(arglist))
  ...
  def parse_pop(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 0 :
      raise Exception('pop does not take arguments, got %d' % len(arglist))
  ...
  def parse_turn(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 1 :
      raise Exception('turn takes 1 argument, got %d' % len(arglist))
  ...
  def parse_bank(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 1 :
      raise Exception('bank takes 1 argument, got %d' % len(arglist))
  ...
  def parse_roll(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 1 :
      raise Exception('roll takes 1 argument, got %d' % len(arglist))
  ...
  def parse_sphere(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 1 :
      raise Exception('sphere takes 1 argument, got %d' % len(arglist))
  ...
  def parse_cylinder(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 2 :
      raise Exception('cylinder takes 2 argument, got %d' % len(arglist))
  ...
  def parse_box(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 3 :
      raise Exception('box takes 3 argument, got %d' % len(arglist))
  ...
  def parse_color(self, symbol) :
    ...
    arglist = self.parse_argument_list([])
    if len(arglist) != 3 :
      raise Exception('color takes 3 argument, got %d' % len(arglist))
  ...
  def parse(self) :
    ...
    if t is None :
      ...
      raise Exception('line %d: expected transsys or lsys, got %s' % (self.scanner.lineno, t))
...
def write_random_transsys(f, name, rtp) :
  raise Exception('write_random_transsys is not implemented anymore')
  #. Use RandomTranssysParameters::generate_transsys() method')
...
def expression_level_dict(transsys_filename, num_timesteps, sampling_period, initial_levels = None) :
  ...
  if type(initial_levels) is float :
    cmd = cmd + '-f %f ' % initial_levels
  elif type(initial_levels) is list :
    glue = ''
    s = ''
  ...
  while line :
    ...
    if line[0] == '#' :
      ...
      if l[1] == 'time' :
        ...
        raise Exception('expression_level_dict: encountered unknown comment "%s"' % line)
    else :
      t = string.atoi(l[0])
      levels = []
      for s in l[1:] :
  ...
  status = p.close()
  if status :
    raise Exception('expression_level_dict: transexpr exit status %d' % status)
...
class ArraySeries(object) :
  ...
  def simulate_timeseries(self, series_name, initial_state, num_timesteps, sampling_period, reference_intensity_func, array_intensity_func) :
    ...
    if initial_state.transsys_program is not self.transsys_program :
      raise Exception('ArraySeries::simulate_timeseries: transsys programs not identical')
    ts = initial_state.time_series(num_timesteps, sampling_period)
    time_list = list(ts.keys())
    time_list.sort()
  ...
  def write_data(self, f, factor_list = None, series_name = None, complete_only = 0, fieldsep = '\t') :
    ...
    if series_name is None :
      array_index_list = list(range(len(self.array_description)))
    else :
      if series_name not in list(self.series_directory.keys()) :
        raise Exception('ArraySeries::write_data: unknown array series "%s"' % series_name)
