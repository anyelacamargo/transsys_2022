...
import random
import string
# import collections # AVCR replaced transsys.utils for utilidad
...
"""
  ...
  def contains(self, other) :
    ...
    if not isinstance(other, Interval) :
      raise Exception('Interval.contains: illegal type')
...
"""
  ...
  if len(active_set) == 0 or len(inactive_set) == 0 :
    return 0.0
  if len(active_set) > 1 :
    m_active, s_active = utilidad.mean_and_stddev(active_set)
  else :
    m_active, s_active = active_set[0], 0.0
  if len(inactive_set) > 1 :
    m_inactive, s_inactive = utilidad.mean_and_stddev(inactive_set)
  else :
...
def flat_symbol_instance_list(lstring_list, transsys_program = None) :
  ...
  for lstring in lstring_list :
    for si in lstring.symbol_list :
      if list(filter(si, transsys_program)) :
        flat_list.append(si)
  return flat_list
...
class Mutator(object) :
  ...
  def mutate(self, seq) :
    if self.insertion_rate != 0.0 or self.deletion_rate != 0.0 :
      raise Exception('insertion / deletion not implemented yet')
    mutseq = ''
...
def hillclimb(objective_function, decoder, initial_dnaseq, num_generations, mutator) :
  ...
  for g in range(num_generations) :
    ...
    d = utilidad.hamming_distance(dnaseq, mutant_dnaseq)
...
"""
  ...
  def parse(self, f) :
    l = f.readline()
    if l == '' :
      raise Exception('expected magic but got EOF')
    l = l.strip()
    if not self.check_savefile_magic(l) :
      raise Exception('bad magic "%s" instead of "%s"' % (l, self.functionName))
...
"""
  ...
  def __str__(self) :
    s = '%s\n' % self.functionName
    s = s + '%s\n' % utilidad.name_value_pair(self.exponentMultiplier, 'exponentMultiplier')
    s = s + '%s\n' % utilidad.name_value_pair(self.coefficient, 'coefficient')
    s = s + '%s\n' % utilidad.name_value_pair(self.minValue, 'minValue')
    return s[:-1]


  def parse_variables(self, f) :
    self.exponentMultiplier = utilidad.parse_float(f, 'exponentMultiplier')
    self.coefficient = utilidad.parse_float(f, 'coefficient')
    self.minValue = utilidad.parse_float(f, 'minValue')
...
"""
    if self.coefficient == 0.0 :
      raise Exception('exponential function with coefficient 0 cannot be inverted')
    if self.exponentMultiplier == 0.0 :
      raise Exception('exponential function with exponentMultiplier 0 cannot be inverted')
    if y < self.minValue :
      return None
...
"""
  ...
  def __str__(self) :
    s = '%s\n' % self.functionName
    s = s + '%s\n' % utilidad.name_value_pair(self.minValue, 'minValue')
    s = s + '%s\n' % utilidad.name_value_pair(self.getMaxValue(), 'maxValue')
    return s[:-1]


  def parse_variables(self, f) :
    self.minValue = utilidad.parse_float(f, 'minValue')
    maxValue = utilidad.parse_float(f, 'maxValue')
...
def parse_transformation_function(f) :
  ...
  for tf in [TransformationFunction(), ExponentialFunction(), SigmoidFunction(), LowerBoundedFunction(), ArctanFunction()] :
    if tf.check_savefile_magic(l) :
      ...
      return tf
  raise Exception('unknown transformation function "%s"' % l)
...
"""
  ...
  def parse(self, f) :
    ...
    l = l.strip()
    if not self.check_savefile_magic(l) :
      raise Exception('bad magic "%s" instead of "%s"' % (l, self.savefile_magic))
    self.parse_variables(f)
...
required by the respective transformer."""
    if self.transsys_program is None :
      raise Exception('no transsys program, cannot clip')
...
"""
    if self.transsys_program is None :
      raise Exception('no transsys program, cannot get parameters')
    return [n.value for n in self.target_node_list]
...
"""
    if self.transsys_program is None :
      raise Exception('no transsys program, cannot set parameters')
    if len(self.target_node_list) != len(parameter_list) :
      raise Exception('parameter list incompatible with transsys program')
...
"""
  ...
  for tf in [ParameterTransformer(), TranssysTypedParameterTransformer()] :
    if tf.check_savefile_magic(l) :
      tf.parse_variables(f)
      return tf
  raise Exception('unknown parameter transformer "%s"' % l)
...
"""
  ...
  def __str__(self) :
    return utilidad.table_row([self.obj, self.obj_alt, self.temperature, self.stepsize, self.stepvector_entropy, self.stepvector_max, self.state_distance, self.p_accept, self.accept])
...
"""
    ...
    gradient_abs = list(map(abs, gradient))
    # print utilidad.euclidean_norm(gradient)
    # print gradient_abs
    self.gradient_entropy = utilidad.shannon_entropy(gradient_abs)
    self.gradient_max = max(gradient_abs)


  def __str__(self) :
    return utilidad.table_row([self.obj, self.stepsize, self.numEvaluations, self.gradient_entropy, self.gradient_max])
...
"""
  ...
  def __init__(self) :
    ...
    raise Exception('cannot instantiate AbstractObjectiveFunction')
...
@param transsys_program: the transsys program to be evaluated
    """
    raise Exception('cannot call AbstractObjectiveFunction')
...
"""
    ...
    while s :
      ...
      factor_name = w[0]
      if factor_name == '' :
        raise Exception('empty factor name')
      if factor_name in list(self.series.keys()) :
        raise Exception('multiple series for factor "%s"' % factor_name)
      self.series[factor_name] = []
      xlist = w[1].split()
      if series_length is None :
        series_length = len(xlist)
      elif series_length != len(xlist) :
        raise Exception('unequal series lengths (%d != %d)' % (series_length, len(xlist)))
      self.series[factor_name] = list(map(float, xlist))
      s = f.readline()
...
expression profiles as an objective function."""
  ...
  def __call__(self, transsys_program) :
    ...
    if l is None :
      raise Exception('call of uninitialised series objective function')
    if l == 0 :
      raise Exception('call of objective function with series length 0')
    ...
    for factor_name in self.series.keys() :
      if transsys_program.find_factor_index(factor_name) == -1 :
        raise Exception('failed to find factor "%s" in transsys "%s"' % (factor_name, transsys_program.name))
    ...
    for factor_name in self.series.keys() :
      ...
      for i in range(l) :
        ...
        if utilidad.is_nan(x) :
          print(str(tseries[i]))
          raise Exception('expression level of factor "%s" is NaN' % factor_name)
        v.append(x)
      sq = utilidad.euclidean_distance_squared(self.series[factor_name], v)
    ...
    for factor_name in self.series.keys() :
      ...
      if min(self.series[factor_name]) < max(self.series[factor_name]) :
        ...
        for i in range(l) :
          ...
          v.append(x)
        if min(v) < max(v) :
          cc = 1.0 - utilidad.uncentered_correlation(self.series[factor_name], v)
          cc_sum = cc_sum + cc
        else :
          cc_sum = cc_sum + 2.0
...
@type gene_name_list: C{list} of C{string}s, or C{None}
"""
    raise Exception('call to unimplemented AbstractOptimiser optimise method')
...
"""
  ...
  def setPerturbationMethod(self, pMethodString) :
    ...
    if pMethodString == 'uniform' :
      ...
      raise Exception('unsupported perturbation method "%s"' % pMethodString)
...
"""
  ...
  def parse_variables(self, f) :
    ...
    self.stepsize_learning_rate = utilidad.parse_float(f, 'stepsize_learning_rate')
    self.target_improvement_ratio = utilidad.parse_float(f, 'target_improvement_ratio')
    self.stepsize_init = utilidad.parse_float(f, 'stepsize_init')
    self.stepvector_learning_rate = utilidad.parse_float(f, 'stepvector_learning_rate')
    self.stepvector_learning_decay = utilidad.parse_float(f, 'stepvector_learning_decay')
    self.cooling_rate = utilidad.parse_float(f, 'cooling_rate')
    self.temperature_init = utilidad.parse_float(f, 'temperature_init', allowNone = True)
    self.termination_temperature = utilidad.parse_float(f, 'termination_temperature', allowNone = True)
    self.termination_objective = utilidad.parse_float(f, 'termination_objective', allowNone = True)
    self.termination_iteration = utilidad.parse_int(f, 'termination_iteration', allowNone = True)
    self.setPerturbationMethod(utilidad.parse_string(f, 'perturbation_method').strip())
  ...
  def __str__(self) :
    s = '%s\n' % self.savefile_magic
    s = s + '%s\n' % utilidad.name_value_pair(self.stepsize_learning_rate, 'stepsize_learning_rate')
    s = s + '%s\n' % utilidad.name_value_pair(self.target_improvement_ratio, 'target_improvement_ratio')
    s = s + '%s\n' % utilidad.name_value_pair(self.stepsize_init, 'stepsize_init')
    s = s + '%s\n' % utilidad.name_value_pair(self.stepvector_learning_rate, 'stepvector_learning_rate')
    s = s + '%s\n' % utilidad.name_value_pair(self.stepvector_learning_decay, 'stepvector_learning_decay')
    s = s + '%s\n' % utilidad.name_value_pair(self.cooling_rate, 'cooling_rate')
    s = s + '%s\n' % utilidad.name_value_pair(self.temperature_init, 'temperature_init')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_temperature, 'termination_temperature')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_objective, 'termination_objective')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_iteration, 'termination_iteration')
    if self.perturbation_method == self.PERTURBATION_METHOD_UNIFORM :
      s = s + '%s\n' % utilidad.name_value_pair('uniform', 'perturbation_method')
    elif self.perturbation_method == self.PERTURBATION_METHOD_GAUSS :
      s = s + '%s\n' % utilidad.name_value_pair('gauss', 'perturbation_method')
    else :
      raise Exception('cannot encode perturbation method')
    s = s + '%s\n' % utilidad.name_value_pair('unknown', 'rndseed')
...
@raise StandardError: No termination condition applicable.
    ...
    if self.termination_iteration is not None :
      return
    raise Exception('no termination condition set')
  ...
  def optimise(self, transsys_program_init, objective_function, factor_name_list = None, gene_name_list = None, stepvector_init = None) :
    ...
    if stepvector_init is None :
      ...
      if len(stepvector_init) != num_dimensions :
        raise Exception('stepvector incompatible with transsys program')
      stepvector = utilidad.normalised_vector(stepvector_init)
    ...
    while not self.terminationCondition(temperature, obj, iteration) :
      ...
      for i in range(num_dimensions) :
        if self.perturbation_method == self.PERTURBATION_METHOD_UNIFORM :
          ...
          raise Exception('unimplemented perturbation method')
      delta_state = []
      for i in range(num_dimensions) :
        delta_state.append(state_alt[i] - state[i])
      state_distance = utilidad.euclidean_norm(delta_state)
      ...
      records.append(SimulatedAnnealingRecord(obj, obj_alt, temperature, stepsize, utilidad.shannon_entropy(stepvector), max(stepvector), state_distance, p_accept, accept))
      if obj_alt <= obj :
        stepsize = stepsize * (1.0 + self.stepsize_learning_rate / self.target_improvement_ratio)
        sdn = utilidad.normalised_vector(delta_state)
        v = []
        for i in range(num_dimensions) :
        ...
        v = [(1.0 - self.stepvector_learning_decay) * x + self.stepvector_learning_decay * unbiased_stepvector_component for x in stepvector]
      stepvector = utilidad.normalised_vector(v)
      if accept :
        if self.verbose :
...
"""
  ...
  def parse_variables(self, f) :
    """Get the values of instance variables from file C{f}."""
    self.initial_stepsize = utilidad.parse_float(f, 'initial_stepsize')
    self.delta = utilidad.parse_float(f, 'delta')
    self.stepsize_shrink = utilidad.parse_float(f, 'stepsize_shrink')
    self.termination_stepsize = utilidad.parse_float(f, 'termination_stepsize', allowNone = True)
    self.termination_objective = utilidad.parse_float(f, 'termination_objective', allowNone = True)
    self.termination_iteration = utilidad.parse_int(f, 'termination_iteration', allowNone = True)
    self.termination_numEvaluations = utilidad.parse_int(f, 'termination_numEvaluations', allowNone = True)
    self.termination_improvement = utilidad.parse_float(f, 'termination_improvement', allowNone = True)
    self.termination_relative_improvement = utilidad.parse_float(f, 'termination_relative_improvement', allowNone = True)
    self.stepsize_max = utilidad.parse_float(f, 'stepsize_max')
    self.eliminateFlatComponents = utilidad.parse_boolean(f, 'eliminateFlatComponents')
    self.transformer = parse_parameter_transformer(f)


  def __str__(self) :
    s = '%s\n' % self.savefile_magic
    s = s + '%s\n' % utilidad.name_value_pair(self.initial_stepsize, 'initial_stepsize')
    s = s + '%s\n' % utilidad.name_value_pair(self.delta, 'delta')
    s = s + '%s\n' % utilidad.name_value_pair(self.stepsize_shrink, 'stepsize_shrink')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_stepsize, 'termination_stepsize')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_objective, 'termination_objective')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_iteration, 'termination_iteration')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_numEvaluations, 'termination_numEvaluations')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_improvement, 'termination_improvement')
    s = s + '%s\n' % utilidad.name_value_pair(self.termination_relative_improvement, 'termination_relative_improvement')
    s = s + '%s\n' % utilidad.name_value_pair(self.stepsize_max, 'stepsize_max')
    s = s + '%s\n' % utilidad.name_value_pair(self.eliminateFlatComponents, 'eliminateFlatComponents')
    s = s + '%s\n' % str(self.transformer)
    return s[:-1]
...
@rtype: bool
    ...
    if self.termination_relative_improvement is not None and improvement is not None :
      ...
      if objective == 0.0 :
        raise Exception('relative improvement diverges because objective is 0.0 (consider setting termination_objective)')
    ...
    if self.termination_numEvaluations is not None :
      if numEvaluations >= self.termination_numEvaluations :
        ...
        return True
    if max(list(map(abs, gradient))) == 0.0 :
      return True
  ...
  def optimise(self, transsys_program, objective_function, factor_name_list = None, gene_name_list = None) :
    ...
    while not self.terminationCondition(stepsize, current_obj, improvement, iteration, numEvaluations, old_gradient) :
      ...
      gradient_norm = utilidad.euclidean_norm(gradient)
...
def parse_optimiser(f) :
  ...
  for o in [GradientOptimiser(), SimulatedAnnealer()] :
    if o.check_savefile_magic(l) :
      o.parse_variables(f)
      return o
  raise Exception('unknown optimiser "%s"' % l)
